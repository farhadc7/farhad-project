...
class SimpleSendSignal:
    ...
    def _trader_(self,signalDto: SignalDto):
        ...
        try:
            ...
            c=0
            while c<10 :
                # _ot = self._reporting._get_open_trades_('{0}_Trader_{1}'
                #                                          .format(signalDto.provider,signalDto.symbol),
                #                                         10)
                # self._zmq._DWX_MTX_GET_ALL_OPEN_TRADES_()               
                # if self._zmq._valid_response_() == False :
                #      c+=1
                #      continue
                # _ot=self._zmq._get_response_() 
                
                # active_list=list(_ot.get('_trades').values())
                # exist_symbol=None
                # for temp in active_list :
                #     if temp.get('_comment') == _default_order['_comment']:
                #         exist_symbol = temp
                
                exists=self._check_exist(_default_order)
 
                if exists == None:
                        
                     #re=self._zmq._DWX_MTX_NEW_TRADE_(_default_order)
                     re=self._execution._execute_(_default_order)
                     sleep(2)
                     
                     order_commited=self._check_exist(_default_order)
                     if self._zmq._valid_response_(re) == False:
                         if order_commited =='not found':
                             print('fuck- order no commited to market')
                     else:
    ...
    def _check_exist(self,_default_order):
        exist_symbol=None
        c=1
        while c<10:
            self._zmq._DWX_MTX_GET_ALL_OPEN_TRADES_()               
