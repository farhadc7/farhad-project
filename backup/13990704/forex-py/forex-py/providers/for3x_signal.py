    ...
    def createSignalDto(self,msg,chName):
        ...
        for line in lines :
            ...
            if line.find('buy') !=-1 or line.find('sell') !=-1:
                ...
                enter= line.replace('limit ','').replace('stop','').split(" ")# EURUSD SELL 1.1840
                ...
                signals[counter].sl =float(line.replace('sl','').replace(' ',''))
                
            elif line.find('tp') !=-1:
                if(signals[counter].tp == 0):
                    signals[counter].tp = float(line.replace('tp','').replace(' ',''))
                elif(signals[counter].tp2 == 0):
                    signals[counter].tp2 = float(line.replace('tp','').replace(' ',''))
                elif(signals[counter].tp3 == 0):
                    signals[counter].tp3 = float(line.replace('tp','').replace(' ',''))
