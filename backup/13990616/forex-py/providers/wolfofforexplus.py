...
class wolfofforexplus:
    ...
    def get_message(self, chName):
        ...
        try:
            ...
            for re in results :
                if( (re[0] != '')) :
                    if(str.find(re[0].lower(),"stop loss") != -1):
    ...
    def createSignalDto(self,msg,chName):
        ...
        for line in lines :
            ...
            posTP=str.find(item.lower(),'stop loss')
            posSL=str.find(item.lower(),'take profit')
            ...
            if posSymbol != -1 : 
                ...
                enter= item.split(' ')
                signalDto.enter_type = 1 if enter[1].lower() == "buy" else 2
                signalDto.enterPrice = float(enter[3])

            elif posSL != -1 :
                signalDto.sl =float(item.split(' ')[4])
                    
            elif posTP != -1 :
                if(signalDto.tp == 0):
                    signalDto.tp = float(item.split(' ')[5])
                elif(signalDto.tp2 == 0):
                    signalDto.tp2 = float(item.split(' ')[5])
                elif(signalDto.tp3 == 0):
                    signalDto.tp3 = float(item.split(' ')[5])
